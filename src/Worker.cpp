...
void AsyncWorker::scheduleFirstOutstandingJob(){
    ...
    queueLock_.unlock();
    {
        std::unique_lock<std::mutex> lock(executingLock_);
        executing_ = true;
        runTheCall(std::move(ctx.action), std::move(ctx.onDone));
        executing_ = false;
        executingCond_.notify_all();
    }
    queueLock_.lock();
}
...
void AsyncWorker::cancelJobsFor(const std::string& name){
    ...
    calls_.erase(std::remove_if(calls_.begin(), calls_.end(), [&](const JobContext& it){
        if (it.id == name){
            std::unique_lock<std::mutex> lock(executingLock_);
            auto isExecuting = [this]()->bool { return !executing_; };
            executingCond_.wait(lock, isExecuting);                
        }
        return true;
    }), calls_.end());
